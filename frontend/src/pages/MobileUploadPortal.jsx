...
const MobileUploadPortal = () => {
  ...
  const handleFileChange = (e, docType) => {
    ...
    // REAL UPLOAD LOGIC: 
    // In a full app, you'd use FormData and axios.post to save the file.
    // For the hackathon, we simulate the upload time:
    setTimeout(() => {
      // 1. Tell the Server (Step 1) that this specific doc is done
    ...
    <div className="min-h-screen bg-slate-50 p-6 font-sans">
      ...
      <div className="space-y-4">
        {["Aadhaar", "Address Proof"].map((doc) => (
          <div key={doc} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            <div className="flex justify-between items-center">
              ...
              {completed.includes(doc) ? (
                ...
                <label className="cursor-pointer">
                  ...
                  />
                  <div className={`p-3 rounded-xl transition-all ${uploading === doc ? 'bg-slate-100 text-slate-400' : 'bg-blue-50 text-blue-600 active:scale-90'}`}>
                    {uploading === doc ? (
      ...
      <div className="mt-10 p-6 bg-blue-600 rounded-3xl text-white shadow-xl text-center">
        ...
        <p className="text-sm opacity-90">Your documents will appear on the big screen instantly after upload.</p>
      </div>
    </div>
  );
};
